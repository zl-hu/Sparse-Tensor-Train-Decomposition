function [x, err]=NTT_RALS(A, r, itemax, rtol, type, lambda, varargin)
...
while  ite <= itemax  &&  err_ite > rtol
    ...
    if loopind <= d
     ...
     %Use active-set method to update the cores and compute relative error    
         ...
         y=zeros(r(ind)*r(ind+1), n(ind));
         for j=1:n(ind)
             y(:, j)=max(lsqnonneg(C, tempA(:,j)), 1e-16);  % The solution maybe zero
         end     
         err_ite=norm(C*y-tempA,'fro')/norm(tempA, 'fro');
         err(ite)=err_ite;     
         y=permute(reshape(y, r(ind), r(ind+1), n(ind)), [1 3 2]);
     else
         error('The fifth parameter should be NNRALS or NNALS!');
     end     
